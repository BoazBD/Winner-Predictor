...
if ! aws lambda get-function --function-name $LAMBDA_FUNCTION_NAME --region $AWS_REGION &> /dev/null; then
    ...
    aws lambda create-function \
        ...
        --timeout 300 \
        --memory-size 2048 \
        --environment "Variables={AWS_REGION=$AWS_REGION,S3_BUCKET=$S3_BUCKET_NAME,MODEL_TYPE=$MODEL_TYPE,EPOCHS=$EPOCHS,MAX_SEQ=$MAX_SEQ,THRESHOLD=$THRESHOLD,ATHENA_DATABASE=$ATHENA_DATABASE,FIRESTORE_PROJECT_ID=$FIRESTORE_PROJECT_ID,GOOGLE_APPLICATION_CREDENTIALS=$GOOGLE_APPLICATION_CREDENTIALS}" \
        --region $AWS_REGION
else
